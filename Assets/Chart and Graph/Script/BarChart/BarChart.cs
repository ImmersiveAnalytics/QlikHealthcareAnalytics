...
namespace ChartAndGraph
{
    ...
    { 
        ...
        /// </summary>
        public BarEvent BarClicked;
        /// <summary>
        /// occurs when a bar is hovered
        /// </summary>
        public BarEvent BarHovered;
            ...
            for(int i=0; i<rowCount; i++)
            {
                for(int j=0; j<columnCount; j++)
                {
                    ...
                    if (mBars.TryGetValue(new ChartItemIndex(i, j), out bar))
                    {
                        ...
                        SetBarSize(bar.BarGameObject, new Vector3(BarSizeLink.Breadth * bar.InitialScale.x, height * bar.InitialScale.y, BarSizeLink.Depth * bar.InitialScale.z));
                        if (bar.Bar != null)
                            bar.Bar.Generate((float)interp);
                        bar.Size = height;
                        bar.Value = amount;
                        FixBarLabels(bar);
                    }
        ...
        private void FixBarLabels(BarObject bar)
        {
            ...
            {
//                Debug.Log(bar.ItemLabel.UIText.text.ToString());
                ...
                Vector3 rotation = new Vector3(45f, 45f, 45f);
                bar.ItemLabel.transform.Rotate(rotation);
            }
        ...
        private GameObject CreateBar(Vector3 innerPosition,double value,float size,float normalizedSize,string category,string group,int index,int categoryIndex)
        {
            ...
            BarGenerator generator = obj.GetComponent<BarGenerator>();
            ...
            if(generator != null)
                generator.Generate(normalizedSize);

            SetBarSize(obj, new Vector3(BarSizeLink.Breadth * initialScale.x, 1f * size * initialScale.y, BarSizeLink.Depth * initialScale.z));
            ...
            if (mItemLabels != null && mItemLabels.isActiveAndEnabled)
            {
                ...
                if (mItemLabels.Alignment == ChartLabelAlignment.Base)
                    angle = -45f;
                BillboardText billboard = ChartCommon.CreateBillboardText(mItemLabels.TextPrefab, topLevel.transform, toSet, labelPos.x, labelPos.y, labelPos.z,angle, obj.transform,hideHierarchy, mItemLabels.FontSize, mItemLabels.FontSharpness);
//                billboard.UserData = 
//                billboard.UIText.fontSize = ItemLabels.FontSize;
            ...
            if(mCategoryLabels != null && mCategoryLabels.isActiveAndEnabled)
            {
                       ...
                       index == 0)
                {
                    ...
                    string toSet = mCategoryLabels.TextFormat.Format(category,"",group);
                    float angle = 45f;
                    if (mCategoryLabels.Alignment == ChartLabelAlignment.Base)
                        angle = -45f;
                    BillboardText billboard = ChartCommon.CreateBillboardText(mCategoryLabels.TextPrefab, topLevel.transform, toSet, labelPos.x, labelPos.y, labelPos.z,angle, obj.transform, hideHierarchy, mCategoryLabels.FontSize, mCategoryLabels.FontSharpness);
...
//                if(mGroupLabels.Alignment == GroupLabelAlignment.)
                BillboardText billboard = ChartCommon.CreateBillboardText(mGroupLabels.TextPrefab, transform, toSet, position.x, position.y, position.z,angle,null, hideHierarchy, mGroupLabels.FontSize, mGroupLabels.FontSharpness);
        ...
        protected override void ClearChart()
        {
            ...
            foreach(BarObject bar in mBars.Values)
            {
                if (bar != null)
                {
                    ...
                    {
                        bar.Bar.Clear();
                        ChartCommon.SafeDestroy(bar.Bar.gameObject);
                    }
